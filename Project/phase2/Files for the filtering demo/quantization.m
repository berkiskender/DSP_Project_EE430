function xq = quantization(x,nbits,maxval)

M = 2^nbits-1;
partition = (0:M-2)/M + 1/(2*M);
codebook = (0:M-1)/M;
partition = partition - (M-1)/(2*M);
codebook  = codebook - (M-1)/(2*M);
partition = maxval*partition;
codebook  = maxval*codebook;

len = length(x);
xq = zeros(size(x));
if size(x,1) == len
    for j = 1:size(x,2)
        [~,xq(:,j)] = quantiz(x(:,j), partition, codebook);
    end
elseif size(x,2) == len
    for i = 1:size(x,1)
        [~,xq(i,:)] = quantiz(x(i,:), partition, codebook);
    end
else
    error('Other dimensions not implemented.');
end 
end
...
if nargout > 2
    ...
    for i = 0 : length(codebook)-1
        distor = distor + sum((sig(find(indx==i)) - codebook(i+1)).^2); %#ok<FNDSB>
    end;
